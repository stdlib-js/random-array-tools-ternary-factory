...
var TernaryRandom = require( '@stdlib/random-array-tools-ternary' );
...
function createFactory( prng, dtypes, dtype ) {
	var isValidDataType;
	if ( !isFunction( prng ) ) {
		throw new TypeError( format( '1ql3c', prng ) );
	}
	if ( !isMethodIn( prng, 'factory' ) ) {
		throw new TypeError( format( '1qlFM', 'factory' ) );
	}
	...
	if ( !isStringArray( dtypes ) ) {
		throw new TypeError( format( '1qlFN', dtypes ) );
	}
	...
	if ( !contains( dtypes, dtype ) ) {
		throw new TypeError( format( '1qlFO', dtype ) );
	}
	...
	function factory() {
		...
		if ( hasOwnProp( opts, 'dtype' ) ) {
			...
			if ( !isValidDataType( dt ) ) {
				throw new TypeError( format( '1ql4S', 'dtype', dtypes.join( '", "' ), dt ) );
			}
		} else {
			dt = dtype;
		}
